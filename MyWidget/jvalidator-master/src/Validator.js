...
define( [ "AsyncRequest", 'RuleParser' ], function ( Async, parser ) {
    var PATTERNS = {}
    var CONSTANT = {
    ...
    var valiTagName = null;// TODO 修改点1 粘着验证元素,以前是form
// ## 字段检查器
// 绑定到某个字段后，对其进行检查等操作
    function FieldChecker( element ) {
        this.element = element;
        this.$element = $( element );
        this.$form = this.$element.closest( valiTagName );
    ...
    FieldChecker.prototype = {
        ...
        _checkPatternResult: function ( str, results ) {
            ...
            if ( all ) {
                return [results[0]];//TODO 6 也返回结果集给成功
            } else {
        ...
        check: function ( $event, checkCallback ) {
            ...
            async.onfinished = function () {
                ...
                var isTrue = errors.length == 1;// TODO 7 判断是否通过
                !isTrue && errors.pop();//移除用于判断的
                self.after_check( isTrue, errors, $event );
                if ( checkCallback ) {
                    checkCallback( isTrue, errors );
                }
            };
        ...
        after_check: function ( is_valid, errors, $event ) {
            ...
            var evt = PATTERNS[errors[0 ].name][type];//TODO 修改点3 调用自定义在(addPattern)里的验证方法
            if ( !evt ) evt = this.$element.data( CONSTANT.FIELD_EVENTS + type );
...
// ## 表单验证器
    ...
    function FormValidator( form ) {
        if ( !form ) throw "[ERROR] 验证元素 必须存在."//TODO 修改点2 去除form验证
        this.form = form;
        this.$form = $( form );
        valiTagName = form.tagName;
        this.async = new Async();
    }

// 判断元素可见并存在
    function _exists( el ) {
        // TODO 修改点4  ||$(el).attr("type")=="hidden" 使其可以验证hidden
...
// 得到指定元素的jvalidator
    function _getFieldValidator( el ) {
        ...
        if ( el.nodeName == "INPUT" && ( el.type == "checkbox" || el.type == "radio" ) ) {
            el = $( el ).closest( valiTagName ).find( "input[data-" + CONSTANT.PATTERN + "][name=" + el.name + "]" )[ 0 ];
        }
        if ( !el ) return;
...
// 设置增加自定义 pattern 的入口
    ...
    function addPattern( name, options ) {
        if ( !name, !options || !options.message || !options.validate )
            throw "[ERROR] add pattern - no param: name,{message:'',validate:Function}"
