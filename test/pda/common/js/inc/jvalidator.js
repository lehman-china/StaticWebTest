/**
 * jvalidator 0.3.9 修改者:雷建军  2015-2-10 10:08:56  532031050
 * @param formSelector
 * @constructor
 */
...
// 解析 when 中的支持 @name 的 selector 语法
    ...
    FormValidator.prototype = {
        ...
        validateAll: function ( validateAllCallback ) {
            ...
            var isContinue = true;// 遇到错误是否继续验证
                    ...
                    async.addRequest( function ( async_continue ) {
                        if ( isContinue ){// 是否继续验证
                            jv.check( null, function ( checkResult, error ) {
                                if ( !checkResult ) {
                                    errors.push( error )
                                    isContinue = continueCheck;// 遇到错误是否继续验证
                                }
                                async_continue();
                            } );
                        } else {
        ...
        setContinueCheck: function ( isContinue ) {//TODO 修改点 17 设置遇到错误是否继续验证, true 遇到错误继续验证,false遇到错误终止验证
            continueCheck = isContinue;
        },
