...
this.defaults.headers.common['X-Requested-With']='XMLHttpRequest';this.defaults.headers.put['Content-Type']='application/x-www-form-urlencoded';this.defaults.headers.post['Content-Type']='application/x-www-form-urlencoded';
this.defaults.transformRequest=[function(a){var b=function(f){var k="";var c,l,g,d,h,j,e;for(c in f){l=f[c];if(l instanceof Array){for(e=0;e<l.length;++e){h=l[e];g=c+"["+e+"]";j={};j[g]=h;k+=b(j)+"&"}}else{if(l instanceof Object){for(d in l){h=l[d];g=c+"["+d+"]";j={};j[g]=h;k+=b(j)+"&"}}else{if(l!==undefined&&l!==null){k+=encodeURIComponent(c)+"="+encodeURIComponent(l)+"&"}}}}return k.length?k.substr(0,k.length-1):k};return angular.isObject(a)&&String(a)!=="[object File]"?b(a):a}];
...
l,e,ga(a));return pa(d,"pattern",d.$isEmpty(c)||e.test(c),c)},d.$formatters.push(e),d.$parsers.push(e));if(c.ngMinlength){var n=S(c.ngMinlength);e=function(a){return pa(d,"minlength",d.$isEmpty(a)||a.length>=n,a)};d.$parsers.push(e);d.$formatters.push(e)}if(c.ngMaxlength){var p=S(c.ngMaxlength);e=function(a){return pa(d,"maxlength",d.$isEmpty(a)||a.length<=p,a)};d.$parsers.push(e);d.$formatters.push(e)}}function Nb(b,a){b="ngClass"+b;return function(){return{restrict:"AC",link:function(c,el,e){function g(b){if(!0===
a||c.$index%2===a){var d=f(b||"");h?ua(b,h)||el.addClass(d).removeClass(h):el.addClass(d)}h=aa(b)}function f(a){if(K(a))return a.join(" ");if(X(a)){var b=[];q(a,function(a,c){a&&b.push(c)});return b.join(" ")}return a}var h;c.$watch(e[b],g,!0);e.$observe("class",function(a){g(c.$eval(e[b]))});"ngClass"!==b&&c.$watch("$index",function(d,g){var h=d&1;if(h!==g&1){var n=f(c.$eval(e[b]));h===a?e.$addClass(n):e.$removeClass(n)}})}}}}var x=function(b){return D(b)?b.toLowerCase():b},Ia=function(b){return D(b)?b.toUpperCase():
...
// 修改,扩展
// 如果,没有引入jquery,则使用 , angular中集成精简jquery
(function() {
	var $2 = angular.element;
	function $t( v ) {
		if ( typeof(v) != "string" ) {
			return $2( v );
		}
		var one = v.charAt( 0 );
		var name = v.substring( 1, v.length );
		if ( one == '#' ) {
			return $2( document.getElementById( name ) );
		} else if ( one == '.' ) {
			return $2( document.getElementsByClassName( name ) );
		} else if ( one == '<' ) {
			return $2( v );
		} else {
			return $2( document.getElementsByTagName( v ) );
		}
	}
	if ( typeof($) == "undefined" )
		jQuery = $ = $t;
...
/**
 * ajax loading 打开:load.open(),关闭:load.open();
 */
function load() {
}
load.$loading = null;
load.open = function( text ) {
	if ( load.$loading )
		return;
	var w = text ? text.length * 12 + 50 : 105;
	var x = $( "body" )[0].offsetWidth / 2 - (w / 2);
	var y = $( "body" )[0].offsetHeight / 2 - 20;
	var overlay = "<div style='height: 100%; width: 100%;left: 0px;top:0px;background-color: #333;position: fixed; z-index: 99998;opacity:0.6;'></div>";
	load.$loading = $( overlay + "<div style='height:42px; width:" + w + "px;left: " + x + "px;top:" + y + "px;background-color: #FFF;position: fixed; z-index: 99999;border-radius: 8px;'><img style='left:5px;top:5px;position:absolute;' src='/bioec/pages/common/images/loading/xubox_loading2.gif'/> <span style='left:45px;top:12px;position:absolute;'>" + (text ? text : '加载中...') + "</span></div>" );
	$( "body" ).append( load.$loading );
};
load.close = function() {
	load.$loading.remove();
	load.$loading = null;
